...
v_min = calculateMinimumVelocity(TARGET_POSE, [P_x, P_y], launch_height) + epilson

s = np.random.normal(v_min, vel_st_dev, 10000)
heading, angle, dist, sqrt_term, y_d = calculateOptimalTrajectoriesNoDrag(TARGET_POSE, [P_x, P_y, launch_height], v_min)

distances = []

for vel in s:
    poses, vel = simulateShotNoDrag(vel, [P_x, P_y, heading], angle, launch_height)
    dist, x, y = findClosestPositionToTarget(poses, TARGET_POSE)
...
print((count/len(distances))*100)

plt.scatter(np.array(s), np.array(distances))
plt.xlabel("Velocities")
plt.ylabel("Distances")
plt.show()
"""
THIS IS AN EXAMPLE OF ITERATING THROUGH THE DIFFERENT POSSIBLE ARCS

angles = []
velocities = []
headings = []
v_min = calculateMinimumVelocity(TARGET_POSE, [P_x, P_y], launch_height) + epilson
v_samples = (MAX_LAUNCH_SPEED - v_min)/DELTA_V
for i in range(int(v_samples)):
    vel = v_min + DELTA_V * i
    heading, angle, dist, sqrt_term, y_d = calculateOptimalTrajectoriesNoDrag(TARGET_POSE, [P_x, P_y, launch_height], vel)
...
ax.scatter(TARGET_POSE[0], TARGET_POSE[1], TARGET_POSE[2], s=50, color='r', label='Target')
for i in range(int(v_samples)):
    poses, vels = simulateShotNoDrag(velocities[i], [P_x, P_y, headings[i]], angles[i], launch_height)
    poses_np = np.array(poses)
    ax.plot(poses_np[:,0], poses_np[:,1], poses_np[:,2])
    max_h = getMaxHeight(poses)
